...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="check_po.py", description="Check that all strings are translated in a .po file"
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for lang in args.lang:
            lang_issues = {}
            for po_ref, po in (
                (client_ref, os.path.join(locale_dir, lang, "wivrn.po")),
                ...
                missing = 0
                for i in po_ref:
                    if not i.obsolete and i.msgid not in entries:
                        missing = missing + 1
                        print(f"::notice file={po}::{lang} translation for {i.msgid!r} is missing")

                        if po not in lang_issues:
                            lang_issues[po] = []
            ...
            if args.manage_issues:
                issues = [
                    i
                    for i in repo.get_issues(state="all", labels=[ISSUE_LABEL])
                    if i.title == ISSUE_TITLE.format(lang=lang)
                ]

                issue_body = "The following translations are missing:\n"
                for po, msgs in lang_issues.items():
