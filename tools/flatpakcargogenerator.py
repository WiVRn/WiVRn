...
async def get_remote_sha256(url: str) -> str:
    logger.info(f"started sha256({url})")
    sha256 = hashlib.sha256()
    async with (
        aiohttp.ClientSession(raise_for_status=True) as http_session,
        http_session.get(url) as response,
    ):
        while True:
...
def fetch_git_repo(git_url: str, commit: str) -> str:
    ...
    subprocess.run(
        ["git", "submodule", "update", "--init", "--recursive"], cwd=clone_dir, check=True
    )
...
async def get_git_repo_packages(git_url: str, commit: str) -> _GitPackagesType:
    logger.info("Loading packages from %s", git_url)
    git_repo_dir = fetch_git_repo(git_url, commit)
    packages: _GitPackagesType = {}

    def get_cargo_toml_packages(root_dir: str, workspace: _TomlType | None = None):
            ...
            if os.path.exists("Cargo.toml"):
                ...
                if "package" in cargo_toml:
                    packages[cargo_toml["package"]["name"]] = _GitPackage(
                        path=os.path.normpath(root_dir), package=cargo_toml, workspace=workspace
                    )
...
async def get_git_package_sources(
    ...
    logger.info("Adding package %s from %s", name, repo_url)
    ...
    git_sources: list[_FlatpakSourceType] = [
        {
            "type": "shell",
            "commands": [f'cp -r --reflink=auto "{pkg_repo_dir}" "{CARGO_CRATES}/{name}"'],
        },
        {
            "type": "inline",
...
async def get_package_sources(
    ...
    git_repos: _GitReposType,
) -> tuple[list[_FlatpakSourceType], _VendorEntryType] | None:
    metadata = cargo_lock.get("metadata")
    name = package["name"]
    version = package["version"]

    if "source" not in package:
        logger.debug("%s has no source", name)
        return None
    source = package["source"]
    ...
    if metadata is not None and key in metadata:
        ...
        logger.warning(f"{name} doesn't have checksum")
        return None
    crate_sources = [
        {
            "type": "archive",
...
async def generate_sources(
    ...
    logger.debug("Adding collected git repos:\n%s", json.dumps(list(git_repos), indent=4))
    git_repo_coros = []
    ...
    sources.extend(itertools.chain.from_iterable(await asyncio.gather(*git_repo_coros)))

    sources.extend(package_sources)

    logger.debug("Vendored sources:\n%s", json.dumps(cargo_vendored_sources, indent=4))
    sources.append(
        {
            "type": "inline",
            "contents": toml.dumps(
...
def main():
    ...
    parser.add_argument(
        "-t", "--git-tarballs", action="store_true", help="Download git repos as tarballs"
    )
