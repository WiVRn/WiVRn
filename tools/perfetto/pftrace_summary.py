...
def summarize(path):
    ...
    for name, dur_list in durations.items():
        if not dur_list or _is_instant(dur_list): continue
...
def compare(path_a, path_b):
    ...
    print(f"\n=== {label_a}  vs  {label_b} ===")
    print(f"  A: {size_a:,} bytes  {span_a/1e9:.3f} s    B: {size_b:,} bytes  {span_b/1e9:.3f} s")

    all_names = sorted(set(dur_a) | set(dur_b))
    span_names = [n for n in all_names
                  if (dur_a.get(n) and not _is_instant(dur_a[n])) or
                     (dur_b.get(n) and not _is_instant(dur_b[n]))]
    ...
    print(f"  {'':32}  {'n':>5}  {'mean ms':>8}  {'p50 ms':>8}  {'p99 ms':>8}"
    ...
    instant_names = [n for n in all_names
                     if (dur_a.get(n) and _is_instant(dur_a[n])) or
                        (dur_b.get(n) and _is_instant(dur_b[n]))]
    if instant_names:
