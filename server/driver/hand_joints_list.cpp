...
static hand_aim_data interpolate_aim(const hand_aim_data & a, const hand_aim_data & b, float t)
{
	...
	result.aim_pose.position = m_vec3_lerp(a.aim_pose.position, b.aim_pose.position, t);

	for (size_t i = 0; i < std::size(result.pinch_strength); i++)
		result.pinch_strength[i] = std::lerp(a.pinch_strength[i], b.pinch_strength[i], t);
