...
template <typename T, typename Enable = void> struct serialization_traits;

class deserialization_error : public std::runtime_error
{
public:
	deserialization_error() : std::runtime_error("Deserialization error") {}
};
...
public:
	deserialization_packet() : read_index(0) {}
	...
	void
	check_remaining_size(size_t min_size) const
	{
...
template <typename T> struct serialization_traits<T, std::enable_if_t<std::is_arithmetic_v<T>>>
{
	static constexpr void
	type_hash(details::hash_context &h)
	{
	...
	static void
	serialize(T value, serialization_packet &packet)
	{
		packet.write(&value, sizeof(value));
	}

	static T
	deserialize(deserialization_packet &packet)
	{
		T value;
...
template <typename T> struct serialization_traits<T, std::enable_if_t<std::is_enum_v<T>>>
{
	static constexpr void
	type_hash(details::hash_context &h)
	{
...
template <class> struct is_stdarray : public std::false_type
{
};

template <class T, size_t N> struct is_stdarray<std::array<T, N>> : public std::true_type
{
};

template <typename T> inline constexpr bool is_stdarray_v = is_stdarray<T>::value;

template <typename T> struct serialization_traits<T, std::enable_if_t<std::is_aggregate_v<T> && !is_stdarray_v<T>>>
{
	template <size_t I>
	static constexpr void
	aux2(details::hash_context &h)
	{
		if constexpr (I > 0)
			h.feed(",");
	...
	template <size_t... I>
	static constexpr void
	aux1(details::hash_context &h, std::index_sequence<I...>)
	{
		(aux2<I>(h), ...);
	}
	...
	static void
	serialize(const T &value, serialization_packet &packet)
	{
...
template <> struct serialization_traits<std::string>
{
	...
	static void
	serialize(const std::string &value, serialization_packet &packet)
	{
...
template <typename T> struct serialization_traits<std::vector<T>>
{
	static constexpr void
	type_hash(details::hash_context &h)
	{
	...
	static void
	serialize(const std::vector<T> &value, serialization_packet &packet)
	{
		packet.serialize<uint64_t>(value.size());
...
template <typename T> struct serialization_traits<std::optional<T>>
{
	static constexpr void
	type_hash(details::hash_context &h)
	{
	...
	static void
	serialize(const std::optional<T> &value, serialization_packet &packet)
	{
		if (value) {
...
template <typename T, size_t N> struct serialization_traits<std::array<T, N>>
{
	static constexpr void
	type_hash(details::hash_context &h)
	{
	...
	static void
	serialize(const std::array<T, N> &value, serialization_packet &packet)
	{
		for (const T &i : value)
			packet.serialize<T>(i);
	}
...
template <typename... T> struct serialization_traits<std::variant<T...>>
{
	template <typename U, typename... V>
	static constexpr void
	type_list_hash(details::hash_context &h)
	{
	...
	static void
	serialize(const std::variant<T...> &value, serialization_packet &packet)
	{
		packet.serialize<uint32_t>(value.index());
		std::visit([&](const auto &x) { packet.serialize(x); }, value);
	}

	template <size_t I> using i_th_type = std::tuple_element_t<I, std::tuple<T...>>;

	template <size_t... I>
	static std::variant<T...>
	deserialize_aux(deserialization_packet &packet, size_t type_index, std::index_sequence<I...>)
	{
		std::variant<T...> value;
...
template <typename Rep, typename Period> struct serialization_traits<std::chrono::duration<Rep, Period>>
{
	static constexpr void
	type_hash(details::hash_context &h)
	{
	...
	static void
	serialize(const std::chrono::duration<Rep, Period> &value, serialization_packet &packet)
	{
