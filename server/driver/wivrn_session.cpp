        ...
        right_hand_interaction(XRT_DEVICE_EXT_HAND_INTERACTION, 1, &hmd, this)
{
	...
	{
		xdevs[xdev_count++] = static_roles.body = &body_tracker.emplace(&hmd, *this, [this](xrt_device & xdev) {
	...
	if (body == from_headset::body_type::htc)
	{
		...
		if (num_generic_trackers > 0)
		{
			...
			{
				xdevs[xdev_count++] = &generic_trackers.emplace_back(std::to_string(i), &hmd, *this);
			}
		}
	}
