...
loaded_image image_loader::do_load_ktx(std::span<const std::byte> bytes, bool srgb, const std::string & name)
{
	...
	if (ktxTexture2_NeedsTranscoding(texture))
	{
		...
		spdlog::warn("KTX2 transcoded to {} in {}", vk::to_string(format.first), std::chrono::duration_cast<std::chrono::microseconds>(dt));
	}
	...
	return loaded_image{
	        .image{libktx_vma_glue::find(vk_texture.allocationId), device, vk_texture.image, name},
	        .image_view{
	                device,
	                vk::ImageViewCreateInfo{
