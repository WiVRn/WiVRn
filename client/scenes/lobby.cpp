...
void scenes::lobby::render(const XrFrameState & frame_state)
{
	...
	if (display_debug_axes)
	{
		...
		for (int i = 0; i < xyz_axes_trackers.size(); i++)
		{
			...
			else if (auto location = application::locate_controller(xr::vive_xr_tracker_spaces[i], world_space, frame_state.predictedDisplayTime))
			{
...
void scenes::lobby::on_focused()
{
	...
	for (int i = 0; i < xr::vive_xr_tracker_spaces.size(); i++)
	{
