...
void scenes::stream::operator()(to_headset::feature_control && control)
{
	switch (control.f)
	{
		case wivrn::to_headset::feature_control::hid_input:
			...
			if (not control.state and (application::get_config().forward_keyboard or application::get_config().forward_mouse))
			{
				auto toast = gui_toast.lock();
				toast->emplace(_("The server does not allow forwarded keyboard and mouse"), true);
				gui_status_last_change = instance.now();
			}
			return;
