		...
		try
		{
			if (pattern_position == pattern.size())
			{
				...
				if (auto locked = tracking_control.lock(); pattern.empty() or not locked->pattern.empty())
				{
					...
					std::ranges::sort(pattern, std::greater{}, [frame_duration](const auto & i) { return -i.prediction_ns % frame_duration; });
			...
			try
			{
				for (; pattern_position < pattern.size(); ++pattern_position)
				{
					...
					if (std::abs(tracking.timestamp - (t0 + item.prediction_ns)) > 1'000'000)
						break;
					...
					switch (item.device)
					{
						...
						case wivrn::device_id::LEFT_HAND:
							if (left_hand)
							{
								auto joints = locate_hands(*left_hand, world_space, tracking.timestamp);
								hands.emplace_back(
								        t0,
								        tracking.timestamp,
						...
						case wivrn::device_id::RIGHT_HAND:
							if (right_hand)
							{
								auto joints = locate_hands(*right_hand, world_space, tracking.timestamp);
								hands.emplace_back(
								        t0,
								        tracking.timestamp,
							...
							std::visit(utils::overloaded{
							                   ...
							                   [&](auto & b) {
								                   body.push_back(from_headset::body_tracking{
								                           ...
								                           .poses = b.locate_spaces(tracking.timestamp, world_space),
								                   });
							                   },
							           },
							           body_tracker);
						...
						case wivrn::device_id::FACE:
							std::visit(utils::overloaded{
							                   [](std::monostate &) {},
							                   [&](auto & ft) {
								                   ft.get_weights(tracking.timestamp, tracking.face.emplace<typename std::remove_reference_t<decltype(ft)>::packet_type>());
							                   },
							           },
							           face_tracker);
						default:
