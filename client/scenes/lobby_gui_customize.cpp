...
std::vector<scenes::lobby::environment_model> scenes::lobby::load_environment_json(const std::string & json)
{
	...
	std::ranges::sort(models, {}, &environment_model::name);
...
scenes::lobby::environment_item_action scenes::lobby::environment_item(environment_model & model, bool download_screenshot)
{
	...
	ImVec2 selectable_size{width, 300};
	...
	if (ImGui::Selectable(("##" + model.gltf_url).c_str(), &selected, /*ImGuiSelectableFlags_AllowOverlap*/ 0, selectable_size))
	{
		if (local)
	...
	ImGui::PushClipRect(backup_cursor + style.FramePadding, backup_cursor + selectable_size - style.FramePadding, true);

	// Display the screenshot
	ImGui::SetCursorScreenPos(backup_cursor + ImVec2(style.FramePadding.x, (selectable_size.y - screenshot_size.y) / 2));
	...
	const float wrap_width = selectable_size.x - indent;
	...
	else if (auto handle = try_get_download_handle(model.gltf_url))
	{
		switch (handle->get_state())
		{
			...
			case libcurl::state::transferring: {
				ImVec2 cancel_size = ImGui::CalcTextSize(_S("Cancel")) + style.FramePadding * 2;
				...
				ImGui::SameLine();
				if (ImGui::Button((_("Cancel") + "##" + model.name).c_str()))
					handle->cancel();
