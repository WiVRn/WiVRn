...
namespace utils
{
	...
	struct future
	{
		...
		template<typename F, typename... Args> friend future<Result, Progress> async(F&& f, Args&&...args);
  ...
  //
	...
	struct async_token
	{
		template<typename R, typename P, typename F, typename... Args> friend future<R, P> async(F&& f, Args&&...args);

		using state = typename future<Result, Progress>::state;
		std::shared_ptr<state> shared_state;

		async_token(std::shared_ptr<state> shared_state) : shared_state(shared_state)
		{
		}
	...
	future<Result, Progress> async(F&& f, Args&&...args)
	{
		...
		fut.shared_state->thread = std::thread([f, args...](auto token)
		{
			try
			{
				token.set_result(std::invoke(f, token, args...));
			}
			catch(...)
			{
