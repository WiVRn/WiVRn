...
void xr::pico_body_tracker::locate_spaces(XrTime time, std::vector<wivrn::from_headset::tracking::pose> & out_poses, XrSpace reference)
{
    ...
    {
        spdlog::warn("xrLocateBodyJointsBD returned {}", xr::to_string(res));
        return;
    }
    ...
    for (auto & joint : joints)
    {
        ...
        if (joint.locationFlags & XR_SPACE_LOCATION_POSITION_TRACKED_BIT)
            pose.flags |= wivrn::from_headset::tracking::position_tracked;
        if (joint.locationFlags & XR_SPACE_LOCATION_ORIENTATION_TRACKED_BIT)
